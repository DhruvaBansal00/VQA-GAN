...
def generate(caption, wordtoix, ixtoword, text_encoder, netG, blob_service, copies=2):
    ...
    if False:
        ...
        text_encoder_out = torch.onnx._export(
            text_encoder,
        ...
        blob_service.create_blob_from_path(
            "models", "text_encoder.onnx", os.path.abspath("text_encoder.onnx")
        )
        print("done")

        print("saving netg.onnx")
        netg_out = torch.onnx._export(
            netG, (noise, sent_emb, words_embs, mask), "netg.onnx", export_params=True
        )
        print("uploading netg.onnx")
        blob_service.create_blob_from_path(
            "models", "netg.onnx", os.path.abspath("netg.onnx")
        )
        print("done")
        return
