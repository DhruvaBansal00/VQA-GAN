...
def get_test_loader(train=False, val=False, test=True):
    """ Returns a data loader for the desired split """
    assert train + val + test == 1, 'need to set exactly one of {train, val, test} to True'
    split = VQA(
        utils.path_for(train=train, val=val, test=test, question=True),
...
if __name__ == "__main__":
    v = VQA(
        utils.path_for(train=True, val=False, test=False, question=True),
        utils.path_for(train=True, val=False, test=False, answer=True),
        config.preprocessed_path,
